...
class PathPlanner:
    ...
    def __init__(self):
        ...
        rospy.init_node("second_path_planner", anonymous=True)
        ...
        # I have added this is very recently (13/02/23) - i need to subscribe to the robot pose, and somehow add into
        #the script the idea that there is an updating start pose - currently there is only an updating goal pose.
        rospy.Subscriber("robot_pose", PoseStamped, self.start_callback)
    ...
    def occupancy_grid_callback(self, map_mssg):
        # Update the map data
    ...
    def goal_callback(self, goal_pose):
        ...
        # so if the map dat ais available then we need to convert the goal pose into grid coords. AND THEN we can call
        # an a_star o rrt* fucntion to perfrom rrt* search. The result of this search is published as a path  message
    ...
    def a_star(self, start, goal):
        ...
        while heap:
            ...
            for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                ...
                # If the neighbor is outside the map, continue
                if x < 0 or x >= self.map_array.shape[0] or y < 0 or y >= self.map_array.shape[1]:
                    continue

                # If the neighbor is an obstacle, continue
                if self.map_array[x, y] == 100:
                    continue

                # Calculate the cost of reaching the neighbor
                g = cost[current] + 1

                # If the cost of reaching the neighbor is lower than the previous cost, update the cost
                if (x, y) not in cost or g < cost[(x, y)]:
                    cost[(x, y)] = g
                    f = g + abs(x - goal[0]) + abs(y - goal[1])
                    heapq.heappush(heap, (f, (x, y)))
                    parent[(x, y)] = current
...
def main():
    # creating a PathPlanner Instance
    PP = PathPlanner()
    rospy.spin()
