...
# The only difference is now the occMSG comes from /map, and then in the method occ_process I need to manually calculate self.odom rel to bottom left (then put -ve signs on them)
# remember move base is being used for OGFM for goal process emthod, but i will need for the frontier stuff to use cart. I can just add a new cart subscriber and method etc.
class GoalSelector:
    ...
    def __init__(self):
        ...
        # Subscribe to OGM
        rospy.Subscriber("/map", OccupancyGrid, self.occ_callback)
        ...
        if self.mean_array is not None or self.var_array is not None:
            ...
            for i in range(self.mean_array.shape[0]):
                for j in range(self.mean_array.shape[1]):
                    ...
                    if 5 <= distance <= self.radius_gas_mapping_state:
    ...
    def goal_process(self):
        try:
            for index in range(self.goals_ordered_matrix.shape[0]):
                ...
                self.x_elem_goal_bot_left = self.goals_ordered_matrix[index][1]
                self.y_elem_goal_bot_left = self.goals_ordered_matrix[index][2]
                # Find coords where cost row is currently relative to bot left mean/var/cart array map.
                x_bot_left_cost = self.goals_ordered_matrix[index][1]*self.resolution_GasMap - self.resolution_GasMap/2
                y_bot_left_cost = self.goals_ordered_matrix[index][2]*self.resolution_GasMap - self.resolution_GasMap/2
                ...
                if 0 < self.mb_occ_array[elem_x_mb_left][elem_y_mb_left] <= 55:
                    # Initialize an empty array with 0 rows and 3 columns
                    self.chosen_goal_info = np.array([self.goals_ordered_matrix[index][0], x_odom_cost, y_odom_cost,
                    ...
                    if index == self.goals_ordered_matrix.shape[0] - 1:
